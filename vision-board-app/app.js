...
function generateBreakdown(title, desc, quarterKeyVal) {
...
document.getElementById('generateBreakdownBtn').addEventListener('click', () => {
  if (!itemTitle.value.trim()) { itemTitle.focus(); return; }
  pendingBreakdown = generateBreakdown(itemTitle.value, itemDesc.value, itemQuarter.value);
  aiSuggestionBody.innerHTML = `
    <div class="suggestion-goal">${escapeHtml(pendingBreakdown.goalStatement)}</div>
    ${pendingBreakdown.milestones.map(m => `
...
document.getElementById('saveItemBtn').addEventListener('click', () => {
  if (!itemTitle.value.trim()) { itemTitle.focus(); return; }
  const goal = pendingBreakdown || generateBreakdown(itemTitle.value, itemDesc.value, itemQuarter.value);
  const item = {
